    ...
    IHttpContextAccessor httpContextAccessor) : ICulturaService
{
    public async Task<List<CulturaResponseDto>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        var query = culturaRepository.Query().Where(x => estufasIds.Contains(x.EstufaId));
        var culturas = await query.ToListAsync(cancellationToken);
        return culturas.Select(x => Map(x, currentUserId)).ToList();
    }
